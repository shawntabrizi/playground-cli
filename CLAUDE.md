...
- **`@dotdm/contracts` is pinned EXACT `3.1.2` and `@dotdm/cdm` EXACT `0.6.15`.** Both shipped breaking changes in PATCH releases (3.1.3 reshaped `CdmJson` — `targets` gone — and removed the `resolveTargetRegistryAddress` export; 0.6.16 rode along), which broke `src/commands/contract.ts` and 6 contract tests under a caret range. Pin exact until we deliberately absorb the new shapes; when bumping, run the contract tests + `tsc` first. Also: the legacy `1.1.1` stable still depends on `@polkadot-apps/*` + PAPI 1.x — do NOT downgrade to it.
