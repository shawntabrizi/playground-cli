...
- `@dotdm/contracts` is on `"latest"` and currently resolves to `1.0.1`, which still hard-depends on the legacy `@polkadot-apps/{contracts,descriptors,chain-client,bulletin,tx}` line + `polkadot-api@1.x`. Until that package migrates to `@parity/product-sdk-*`, our process loads two PAPI majors side by side: `1.23.3` (only reachable via `@dotdm/contracts`'s CDM contract path) and `2.1.2` (everything else).
- `polkadot-api` is pinned to `^2.1.2` and `@polkadot-api/sdk-ink` to `^0.7.0`. Don't bump further until `@dotdm/contracts` migrates — the structural cast that lets us hand a PAPI 2.x client to `@dotdm/contracts`'s `PipelineChainClient` consumer (`src/utils/deploy/run.ts:293`) depends on the current shape compat.
...
- **QR-paired session signer** (`src/utils/auth.ts::createPlaygroundSigner`) — the published `@parity/product-sdk-terminal@0.1.0`'s `createSessionSignerForAccount` routes every signing op through `session.signRaw`, which the Polkadot mobile app wraps with `<Bytes>…</Bytes>` (producing `BadProof` on-chain for transactions). We instead build a PJS signer directly via `getPolkadotSignerFromPjs` with split callbacks: tx → `session.signPayload` (chain-tx context, no envelope), bytes → `session.signRaw` (envelope applied, correct for arbitrary data). The product account derives from `productId="playground.dot"` + `derivationIndex=0`, which matches what the playground-app uses on the host side so the same address signs in both surfaces. The upstream fix is on `paritytech/product-sdk` `main` (commit `a33edf3`); switch back to `createSessionSignerForAccount` once it ships on npm.
